...
int main()
{
  ...
  s1 = spi_in_short(spi_if);
  s2 = spi_in_short(spi_if);
  b2 = spi_in_byte(spi_if);
  printstr("0x");
  printhexln(s1);
  printstr("0x");
  printhexln(s2);
  ...
  buffer[3] = s2 & 0xFF;
  buffer[4] = b2;
  spi_out_buffer(spi_if, buffer, 5);
  ...
  spi_out_buffer(spi_if, (w, unsigned char[]), 68);
